            ...
            if ($row['add_card'] == 0) {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $resultl = mysqli_stmt_get_result($result);
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            $sql = "SELECT device_department FROM devices WHERE device_uid=?";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                echo "SQL_Error";
                                exit();
                                ...
                                $resultl = mysqli_stmt_get_result($result);
                                if ($row = mysqli_fetch_assoc($resultl)) {
                                    $dev_name = $row['device_department'];
                                }
                                else{
                                    $dev_name = "All";
                                }
                            }
                            $sql="UPDATE students SET username=?, serialnumber=?, gender=?, email=?, user_date=CURDATE(), device_uid=?, device_department=?, add_card=1 WHERE id=?";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email)) {
                                ...
                                $sql="UPDATE students SET username=?, serialnumber=?, gender=?, email=?, device_uid=?, device_department=? WHERE id=?";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
