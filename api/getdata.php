                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    if ($row = mysqli_fetch_assoc($resultl)){
                        ...
                        if ($row['device_uid'] == $device_uid || $row['device_uid'] == 0){
                                ...
                                $sql = "SELECT * FROM logs WHERE card_uid=? AND checkindate=? AND card_out=0";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    ...
                                    if (!$row = mysqli_fetch_assoc($resultl)){
                                        ...
                                        $sql = "INSERT INTO logs (username, serialnumber, card_uid, device_uid, device_dep, checkindate, timein, timeout) VALUES (? ,?, ?, ?, ?, ?, ?, ?)";
                                        $result = mysqli_stmt_init($conn);
                                        if (!mysqli_stmt_prepare($result, $sql)) {
                                    ...
                                    //Logout
                                        ...
                                        $sql="UPDATE logs SET timeout=?, card_out=1 WHERE card_uid=? AND checkindate=? AND card_out=0";
                                        $result = mysqli_stmt_init($conn);
                                        if (!mysqli_stmt_prepare($result, $sql)) {
