            ...
            if ($row = mysqli_fetch_assoc($resultl)) {
                ...
                if ($pwdCheck == false) {
                    ...
                    if ($useremail == $up_email) {
                        $sql = "UPDATE students SET user_name=? WHERE user_email=?";
                        $stmt = mysqli_stmt_init($conn);
                        if (!mysqli_stmt_prepare($stmt, $sql)) {
                        ...
                        $sql = "SELECT user_email FROM students WHERE user_email=?";  
                        $result = mysqli_stmt_init($conn);
                        if ( !mysqli_stmt_prepare($result, $sql)){
                            header("location: account.php?error=sqlerror1");
                            exit();
                            ...
                            $resultl = mysqli_stmt_get_result($result);
                            if (!$row = mysqli_fetch_assoc($resultl)) {
                                $sql = "UPDATE students SET user_name=?, user_email=? WHERE user_email=?";
                                $stmt = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($stmt, $sql)) {
                                    header("location: account.php?error=sqlerror");
                                    exit();
